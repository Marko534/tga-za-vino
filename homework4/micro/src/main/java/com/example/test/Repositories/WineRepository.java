...
public interface WineRepository extends CrudRepository<Wine, UUID> {
    ...
    @Query(value = "SELECT * " +
            ...
            +
            "OR (price LIKE '€ %' AND (CAST(REPLACE(SUBSTRING(price, POSITION(' ' IN price)), ',00', '') AS INT) * 61) BETWEEN :minPrice AND :maxPrice) ORDER BY name", nativeQuery = true)
    ...
    @Query(value = "SELECT * " +
            ...
            +
            "OR (price LIKE '€ %' AND (CAST(REPLACE(SUBSTRING(price, POSITION(' ' IN price)), ',00', '') AS INT) * 61) BETWEEN :minPrice AND :maxPrice)) ORDER BY name", nativeQuery = true)
    List<Wine> findByPriceIntervalAndNameContainingIgnoreCase(@Param("wName") String wName,
