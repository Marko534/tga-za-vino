        ...
        <div className={'flex flex-col items-center justify-center gap-4 h-full w-full'}>
          ...
          <div className={'w-full rounded-lg p-8 h-full flex flex-col items-start justify-start gap-4 bg-white/50'}>
            ...
            {searchResults?.wines &&
              searchResults.wines.map((wine: Wine) => (
                <Link to={`/${wine.id}`} key={wine.id} className={'group'}>
                  <div
                    className={
                      'flex flex-row items-center justify-start gap-2 w-full group-hover:scale-105 transition-all ease-in-out duration-105 text-white'
                    }
                  >
            ...
            {searchResults?.wineries &&
              searchResults.wineries.map((winery: Winery) => (
                <Link to={`/winery/${winery.id}`} key={winery.id} className={'group'}>
                  <div
                    className={
                      'flex flex-row items-center jus tify-start gap-2 w-full group-hover:scale-105 transition-all ease-in-out duration-105 text-white'
                    }
                  >
